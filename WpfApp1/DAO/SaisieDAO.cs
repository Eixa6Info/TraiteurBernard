...
namespace TraiteurBernardWPF.DAO
{
    class SaisieDAO
    {
        ...
        /// <param name="db"></param>
        /// <returns></returns>
        internal static List<Saisie> getAllFromYearWeekDayForTourneeForPersonne(Personne personne, string tournee1, string tournee2, int anneee, int semaine, int jour, BaseContext db)
        {
            //BaseContext db = new BaseContext();
            IQueryable<Saisie> req = from s in db.Saisies
                                     where s.Jour == jour && s.Annee == anneee && s.Semaine == semaine && (s.Tournee.Nom == tournee1 || s.Tournee.Nom == tournee2) && s.Personne == personne
                                     select s;
